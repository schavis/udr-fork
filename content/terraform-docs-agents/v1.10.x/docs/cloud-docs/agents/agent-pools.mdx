...
## Exclude an Agent Pool from Specific Workspaces

Excluding an agent pool lets you restrict access to agent pools for workspaces.

To exclude an agent pool from specific workspaces within the organization:
...
Scoping an agent pool lets you specify which projects can use it. The agent pool is only available from within the chosen projects, and it does not appear other projects's lists of available agent pools.

~> **Important:** Scoping an agent pool does not remove it from projects that are actively using it. To remove access, you must first remove the agent pool from within the project's settings.
...
Use the following steps to configure a project to use an agent pool.
...
The project begins using the agent for Terraform runs. Runs involving an agent include information about that agent in the run details. Terraform Cloud may use different agents for the plan and apply operations, depending on agent availability within the pool.
