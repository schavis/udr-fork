...
You can deploy agents to x86_64 or ARM64 architecture to match the architecture of the providers you use to provision infrastructure. Because most provider versions in the public registry do not support ARM64, we recommend deploying x86_64 agents. <!--revisit this statement in the next version /--> 

Agents download Terraform and providers for the specific architecture in order to execute Terraform operations, such as plan and apply commands. Because agents on ARM cannot execute x86 binaries, and agents on x86 cannot execute ARM binaries, all agents within a pool must have the same architecture to prevent Terraform from sending configuration to an agent with an incompatible architecture.

The architecture of the agent pool is determined by the first agent that connects to Terraform. For example, when the first agent to register to an empty pool is an ARM agent, subsequent agents that register must be ARM architecture agents. Attempting to register an ARM agent to an existing x86-only agent pool results in an error.
...
- Agents on x86-64 architecture supports Terraform 0.12 and newer.
- Agents on ARM64 architecture supports Terraform 0.13.5 and newer.
