...
If your organization primarily uses providers that are compiled and available for ARM, then you can consider switching all of your agent pools to agents running on ARM.

You can call the [Terraform provider registry API](/terraform/internals/provider-registry-protocol#list-available-versions) to cross-reference providers against the registry for available architectures. 
