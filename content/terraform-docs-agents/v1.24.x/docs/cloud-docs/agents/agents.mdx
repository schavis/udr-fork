...
You can create queries to search for existing infrastructure for resources so that you can import them to your Terraform workspace. To use the search functionality, you must include the `query` operation in the [`TFE_AGENT_ACCEPT` variable](#accept) when starting your agent. Refer to [Import existing resources to state](terraform/cloud-docs/workspaces/import) for more information. 
