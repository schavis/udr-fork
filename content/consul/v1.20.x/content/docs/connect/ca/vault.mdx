...
last_modified: 2026-03-02T10:05:45.000Z
...
- `AuthMethod` / `auth_method` (`map: nil`) - Vault auth method to use for logging in to Vault.
  ...
  **Security (CVE-2026-2808):** As of Consul 1.22.x, credential files referenced by auth method
  ...
  Credential files outside these directories will be rejected. Symlinks that escape
  these directories are also blocked. If your deployment stores credential files in
  non-standard paths, you must relocate them to one of the listed allowed directories
  before upgrading.
