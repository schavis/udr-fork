...
last_modified: 2026-02-23T11:12:30.000Z
...
To disable prefixing the hostname, set `telemetry.disable_hostname=true` in the [agent configuration](/consul/docs/agent/config/config-files#telemetry-disable_hostname).

You can also send Consul telemetry to a [statsite](http://github.com/armon/statsite) or [statsd](http://github.com/etsy/statsd) server where it can be aggregated and flushed to Graphite or any other metrics store. 
Configure [`telemetry` parameters in the agent configuration](/consul/docs/agent/config/config-files#telemetry-parameters) to use this option.
...
For a full list of metrics emitted by Consul, refer to the [Telemetry reference documentation](/consul/docs/agent/monitor/telemetry-metrics).
...
Servers retain [`raft_trailing_logs`](/consul/docs/agent/config/config-files#raft_trailing_logs) (default `10240`) log entries even if their snapshot was more recent. 
...
Since Consul `1.5.3` [`raft_trailing_logs`](/consul/docs/agent/config/config-files#raft_trailing_logs) has been configurable. 
...
Since Consul `1.10.0` [`raft_trailing_logs`](/consul/docs/agent/config/config-files#raft_trailing_logs) is now reloadable with `consul reload` or `SIGHUP` allowing operators to increase this without the leader restarting or loosing leadership allowing the cluster to be recovered gracefully.
...
If Bolt DB log storage performance becomes an issue and is caused by free list management then setting [`raft_logstore.boltdb.no_freelist_sync`](/consul/docs/agent/config/config-files#raft_logstore_boltdb_no_freelist_sync) to `true` in the server's configuration may help to reduce disk IO and log storage operation times. 
