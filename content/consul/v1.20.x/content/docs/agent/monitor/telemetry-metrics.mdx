...
created_at: 2026-02-23T11:12:30.000Z
...
For information about service mesh traffic metrics, refer to [Observe service mesh telemetry](/consul/docs/connect/observability).

## ACL metrics

Metrics related to the [ACL subsystem](/consul/docs/security/acl) that measure times needed for ACL operations as well as blocked operations.
...
Metrics relative to the Consul [catalog](/consul/docs/architecture/catalog) and [KV store](/consul/docs/dynamic-app-config/kv), containing information about the amount of requests and the performance of the queries.
...
Metrics relative to the performances of the [Serf gossip protocol](/consul/docs/architecture/gossip), used to manage membership and broadcast messages to the cluster.
...
[Cluster peering](/consul/docs/connect/cluster-peering) refers to Consul clusters that communicate through a peer connection, as opposed to a federated connection. 
...
Metrics relative to the [Raft protocol](/consul/docs/architecture/consensus) that is used to maintain consensus in the Consul cluster. 
These metrics include the ones for the Raft's LogStore that can be either Bolt DB or [WAL](/consul/docs/agent/wal-logstore). 
...
Metrics relative to the performances of Consul [sessions](/consul/docs/dynamic-app-config/sessions).
