...
* Providers in component configurations support the [`for_each`](/terraform/language/meta-arguments/for_each) meta argument.
...
Define your Stack’s `provider` blocks in a top-level `.tfcomponent.hcl` file. The following example file named `providers.tfcomponent.hcl` defines an AWS provider.
...
After configuring your provider, you can use the Terraform Stacks CLI to [generate a provider lock file](/terraform/language/stacks/reference/tfcomponents-cli#tfcomponent-providers-lock-command).
...
A Stack cannot run without a lock file for its providers. After defining your providers, use the Terraform Stacks CLI to generate a `.terraform.lock.hcl` lock file. The `tfcomponents providers lock` command creates and updates your `.terraform.lock.hcl` file.

```shell-session
$ tfcomponents providers lock
```

The `tfcomponents providers lock` command uses the `required_providers` block from your configuration to download the listed providers and compute the provider lock hashes. The `tfcomponents providers lock` command only checks the `required_providers` block, so you must list all of the providers you use in that block to ensure that the `tfcomponents` CLI can find them.

If you update your Stack’s providers, you must rerun the `tfcomponents providers lock` command to update your Stack’s providers lock file. For more details, refer to [Terraform Stacks CLI](/terraform/language/stacks/reference/tfcomponents-cli).

## Next steps

After declaring your providers, you can either finish [defining the rest of your component configuration](/terraform/language/stacks/component/config) or move on to [define your Stack’s deployment configuration](/terraform/language/stacks/deploy/config).
