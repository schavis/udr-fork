...
- Providers in component configurations support the [`for_each`](/terraform/language/block/stack/component_configuration/provider#for_each) meta-argument.
...
For more details on the `required_providers` block in Stacks, refer to the [`required_providers` block reference](/terraform/language/block/stack/component_configuration/required_providers). For more details on the `provider` block in Stacks, refer to the [`provider` block reference](/terraform/language/block/stack/component_configuration/provider).
