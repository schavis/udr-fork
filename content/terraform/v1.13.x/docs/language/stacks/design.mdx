...
To keep your component configuration concise, use the [locals block](/terraform/language/values/locals) to share values across your deployments. For example, the following `locals` block defines a variable for you `tf_organization` and `tf_project_name` to use in multiple deployments:
...
Additionally, using meta-arguments like `for_each` in your configuration can help streamline the creation of multiple components, making your configuration more flexible and scalable. Refer to the [`component` block documentation](/terraform/language/block/stacks/component/component#for_each) for more details.
...
After planning out your Stack, learn how to make your design a reality by [defining a component configuration](/terraform/language/block/stack/component_configuration/component#for_each).
