...
- `-invoke=action.<TYPE>.<LABEL>`: Specifies an action to create a plan for. When set, Terraform exclucdes all other configurations from the plan.
...
The following command sets the `env` input variable to `prod` so that the plan can model the operation that requires the input: 
...
The following command creates a plan that only models an `aws_lambda_invoke` action named `test`:
