...
The following example declares an `aws_lambda_invoke` action named `example`:
...
Refer to the [AWS provider documentation](https://registry.terraform.io/providers/hashicorp/aws/6.14.0/docs/resources/lambda_function#basic-function-with-nodejs) for information about provisioning AWS Lambda functions.
...
In the following example, Terraform invokes the action three times per the `count` meta-argment. It also uses the provider configuration reference with `aws.alias` per the `provider` meta-argument. Refer to the [`action` block reference](/terraform/language/block/action) for information about all meta-arguments supported in the `action` block. 
...
The following example creates a plan to model the `aws_lambda_invocation.example` action:
...
Configure an `action_trigger` block and specify the action to run and the events that trigger the action. The `action_trigger` block is a member of the `lifecycle` block nested in a `resource` block. Refer to the [`action_trigger` documentation](/terraform/language/block/resource#action_trigger) for details about configuring the block.

In the following example, Terraform invokes the `action.aws_lambda_invoke.example` action after creating the `aws_lambda_function.example` resource. Terraform also invokes action during a `terraform apply` operation if the `resource` block configuration changes.
...
Terraform also invokes the action if you update the `resource` block configuration and run `terraform apply` again. Terraform doesn't invoke the action if you run the command again without updating the `resource` block configuration. 
