...
Terraform also updates the resource on `terraform apply` without changes to the configuration when it detects drift in the resource that would trigger an `after_update` action. 
