...
Stacks use the [`identity_token` block](/terraform/language/block/stack/deploy/identity_token) to define a JSON Web Token (JWT) for a specific deployment. This token enables OIDC-based authentication, allowing your Stack deployments to securely connect to cloud providers like AWS, Azure, and GCP.
