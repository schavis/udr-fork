...
Terraform v1.12 or newer is required to check the discovered resources' identities, which lets HCP Terraform find resources in other workspaces.
...
If HCP Terraform is connected to your source control system, you can also check your configurations into your VCS before running the query in HCP Terraform.  Refer to [Connect to VCS Providers](/terraform/cloud-docs/vcs) for more information.
...
After importing your resources, you can discard the query configuration file and the `generated.tf` file, but you can archive them to maintain a historical record of the import.