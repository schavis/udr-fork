...
Only configurations using Cassandra's `PasswordAuthenticator` are supported.

The following command executes the Cassandra client and establishes the connection through Boundary:

```shell-session
$ boundary connect cassandra -target-id ttcp_1234567890
```
...
The Mongo helper authorizes a session against a target and uses `mongosh` to invoke a MongoDB client to automatically connect with the appropriate connection parameters and credentials.

The following command executes the MongoDB client and establishes the connection through Boundary:

```shell-session
$ boundary connect mongo -target-id ttcp_1234567890
```

The Mongo connect helper supports the following flags:
...
The MySQL helper authorizes a session against a target and invokes a MySQL client to automatically connect with the appropriate connection parameters and credentials.

The following command executes the MySQL client and establishes the connection through Boundary:
...
The Redis helper authorizes a session against a target and invokes a Redis client to automatically connect with the appropriate connection parameters and credentials.
Only username/password credentials are supported.

The following command executes the Redis client and establishes the connection through Boundary:

```shell-session
$ boundary connect redis -target-id ttcp_1234567890
```

The Redis connect helper supports the following flags:
