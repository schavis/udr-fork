...
- `output-client-agent-cli-error` - If set to `true`, Boundary outputs any CLI errors that are encountered during Client Agent call backs.
The default is `false`.
You can also specify the whether Boundary should output the errors using the **BOUNDARY_CLIENT_AGENT_CLI_ERROR_OUTPUT** environment variable.
