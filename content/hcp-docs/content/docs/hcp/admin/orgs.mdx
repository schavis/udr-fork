...
Organizations must be deleted by HashiCorp Support. Organization owners can use the deletion checklist to ensure all prerequisites are met, which speeds up the deletion process and reduces back-and-forth communication with HashiCorp Support. 

To access the deletion checklist:

1. From the **Organization dashboard**, click **Organization settings**.
1. From the **Manage** menu, select **Deletion checklist**.
1. The **Deletion checklist** page displays the requirements for deleting the organization across the following categories:
   - **Billing:** Requirements include having a single default billing account with a valid status, not being part of an active Flex or annual contract, having no unpaid invoices, and no active billable resources.
   - **Identity & Access Management (IAM):** Requirements include having no users (other than the requesting owner), no user groups, no service principals, and having SSO and SCIM disabled.
   - **Resources:** Requirements include having no more than one project and no active project resources.
1. For each requirement, the checklist displays a status of `Done` or `To Do`. For any `To Do` items, click the ellipsis (**...**) and select the provided action to view details and resolve the blocking issue.
