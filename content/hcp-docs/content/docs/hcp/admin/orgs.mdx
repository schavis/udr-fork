...
1. The **Deletion checklist** page displays the requirements for deleting the organization across the following categories:
   - **Billing:** Requirements include not being part of an active Flex or annual contract, having no unpaid invoices, and no active billable resources.
   - **Identity & Access Management (IAM):** Requirements include having no users (other than the requesting owner), no user groups, no service principals, and having SSO disabled.
