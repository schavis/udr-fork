...
created_at: 2026-03-05T16:17:42.000Z
last_modified: 2026-03-05T16:17:42.000Z
...
## Resource Identity Behaviors

~> **NOTE:** Resource Identity is only supported in Terraform `v1.12` or higher

If the schema being defined represents an [identity](/terraform/plugin/sdkv2/resources/identity) for a managed resource,
set either `OptionalForImport` or `RequiredForImport` to `true` to inform Terraform if practitioners must set the attribute
when importing with that resource's identity.

Unlike the resource state schema, the resource identity schema does not support behaviors such as `Required` and `Computed`.
All identity data is set by the provider, so the entire object is treated as `Computed`. The only behaviors allowed for identity schema
are `OptionalForImport` or `RequiredForImport`, with the resulting behavior of:
...
import {
  ...
  identity = {
    id = "id-123"
    # region can be omitted or supplied
  }
}
```
