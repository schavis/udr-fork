...
~> **NOTE:** List is only supported in Terraform `v1.14` or higher

List resources are an abstraction that allows Terraform to search for a specific resource type within a given scope. List resources are not natively supported in SDKv2, but there are compatibility implementations available to support the use of the framework interface on SDKv2 resources.

The [list](/terraform/plugin/sdkv2/resources/list) page discusses how to add list to an SDKv2 resource.