...
The Terraform MCP server runs in a Docker container that you can deploy to either your local workstation or a remote instance. When running the server locally, start it in the default `stdio` mode. In this mode, the server receives and sends messages in JSON format over RPC. When running the server on a remote instance, start the server in `streamable-http` mode so that you can communicate with the server using the Streamable HTTP transport.
...
- Install compiled the binary into your client. The binaries are available at [releases.hashicorp.com](https://releases.hashicorp.com/terraform-mcp-server/).
...
```shell-session
$ go install go install github.com/hashicorp/terraform-mcp-server/cmd/terraform-mcp-server@main
```
