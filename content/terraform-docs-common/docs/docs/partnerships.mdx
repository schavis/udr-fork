...
The Terraform Integration Program facilitates prospect and existing partners in creating and publishing Terraform integrations validated by HashiCorp.
...
**Technical:**
- Include an SBOM for the provider using [Syft](https://github.com/anchore/syft), the [CycloneDX Generator](https://github.com/CycloneDX/cdxgen), or other SBOM generation tool.

**Feature:**
- [Terraform Checks](https://developer.hashicorp.com/terraform/language/validate) for resource validation.
...
- [Terraform Search](TBD)
- [Terraform Actions](TBD)
...
Beyond run tasks, partners should leverage the [APIs in HCP Terraform](https://developer.hashicorp.com/terraform/cloud-docs/api-docs) or [APIs in Terraform Enterprise](https://developer.hashicorp.com/terraform/enterprise/api-docs) to programmatically manage most aspects of the platform and their accompanying integrations.

Please note to consider network access configuration when integrating with Terraform Enterprise, specifically around configuring [secondary hostnames](https://developer.hashicorp.com/terraform/enterprise/deploy/reference/configuration#tfe_hostname_secondary) that allow Terraform Enterprise to connect to and integrate with external systems when deployed in a private environment
