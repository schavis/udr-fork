...
For example, if a team has **Manage all workspaces** permissions at the organization level, and  **Read** permission for a specific workspace, then the team has **Manage all workspaces** permissions for that workspace. An organization-level permission does not take precedence over a workspace-level permission.
