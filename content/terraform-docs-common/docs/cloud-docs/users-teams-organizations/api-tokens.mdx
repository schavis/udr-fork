...
last_modified: 2026-03-05T21:39:34.000Z
...
We recommend creating tokens that are valid for the shortest duration possible. Tokens with a long time to live increases your security risk when, for example, you forget to delete tokens once their intended purpose is complete.
...
Organization owners can set a maximum token expiration time in the organization's settings. For more information, refer to the [Maximum Time to Live](/terraform/cloud-docs/users-teams-organizations/organizations#maximum-time-to-live) documentation.