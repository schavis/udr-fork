...
<EnterpriseAlert>For this feature to function correctly in on-premise environments, any intermediate infrastructure — such as load balancers, reverse proxies, or Web Application Firewalls (WAFs) — must be configured to preserve the original client IP address. Ensure your network equipment is set to pass the originating IP through the X-Forwarded-For HTTP header so the application can accurately validate incoming traffic against your allowlists.</EnterpriseAlert>
