...
HCP Terraform can use webhooks to notify external systems about events. Team notifications allow you to configure relevant alerts that notify teams you specify whenever a certain event occurs. Team notifications can currently only be configured to send alerts about [change requests](/terraform/cloud-docs/workspaces/change-requests).
