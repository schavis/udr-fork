...
These instructions are for using repositories from GitHub.com with HCP Terraform workspaces or Stacks and private registry modules, without requiring an organization owner to configure an OAuth connection.
...
This means that different HCP Terraform users within the same organization can see different sets of repositories available for their workspaces or Stacks.
...
If you disallow access to a repository that is currently connected to any HCP Terraform workspaces or Stacks, those workspaces or Stacks are unable to retrieve configuration versions until you change their VCS settings and connect them to an allowed repository.
