...
To disable remote execution for a project, enable the **Local** execution. This mode lets you perform Terraform runs locally with the [CLI-driven run workflow](/terraform/cloud-docs/run/cli).
...
1. Disable [auto-apply](/terraform/cloud-docs/workspaces/settings#auto-apply) if it is enabled.
...
1. Enable [auto-apply](/terraform/cloud-docs/workspaces/settings#auto-apply), if it was enabled before changing your execution mode.
