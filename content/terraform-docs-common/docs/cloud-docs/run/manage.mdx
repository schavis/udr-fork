...
If a run invokes an action, HCP Terraform adds the **action:** label and an icon. Actions are preset operations built into providers that let Terraform perform day-two operations. Refer to [Invoke an action](/terraform/language/v1.14.x/invoke-actions) in the Terraform configuration language documentation for more information. 
