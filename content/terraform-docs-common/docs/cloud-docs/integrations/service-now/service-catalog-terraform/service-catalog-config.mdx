...
Version 2.9.0 introduces an approval step in the flow. The approval flow is an additional step that ensures requests are reviewed and authorized before resources can be created.
This new feature gives customers more control over their customized solutions. The overall idea is simple to our exisiting catalog items, resources are ordered, a terraform run is queued (speculative plan) and the flow is paused at this stage for the approver 
to review the request and the plan in HCP Terraform. Upon approval, the plan is applied. However, if it is rejected, the plan will not be applied.

This is done by adding two new catalog items: 

   - Provision Resources with Vars with Approval
   - Update Resources with Vars with Approval

We recommend customizing the approval flow with the appropriate approver. Refer to the [example customizations] (/terraform/cloud-docs/integrations/service-now/service-catalog-terraform/example-customizations#Make-a-copy-of-the-existing-Catalog-Item)for more details.