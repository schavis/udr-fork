...
| Provision Resources with Vars with Approval | Creates a new HCP Terraform workspace (with auto-apply), creates any variables, creates/queues a speculative plan, pauses for approval. If approved the run is applied, if rejected the run will not be applied.  
