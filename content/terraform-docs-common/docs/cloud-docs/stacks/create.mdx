...
* A repository with a top-level [Stack configuration](/terraform/language/stacks/create/config) and a [deployment configuration](/terraform/language/stacks/deploy/config). Your component configuration file should include at least one `component` block, and your deployment configuration file should include at least one `deployment` block.
