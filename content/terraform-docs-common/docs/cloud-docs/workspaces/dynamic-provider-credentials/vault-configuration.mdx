...
#### Token TTLs
We recommend creating a role which issues a renewable token, and setting token_ttl to a relatively short value. HCP Terraform can renew the token periodically until the plan or apply is complete, then revoke it to prevent it from being used further.

If a nonrenewable token is used, then it is required that the TTL of the token matches or exceeds than the [Terraform Run phase timeout](https://developer.hashicorp.com/terraform/enterprise/application-administration/general#terraform-run-timeout-settings)
