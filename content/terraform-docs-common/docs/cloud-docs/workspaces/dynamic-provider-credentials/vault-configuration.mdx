...
If a nonrenewable token is used, then it is required that the TTL of the token matches or exceeds the [Terraform Run phase timeout](/terraform/enterprise/application-administration/general#terraform-run-timeout-settings).
