...
You can define queries in your Terraform configuration to search for resources that are not yet managed by Terraform. HCP Terraform presents query results so that you can use the UI to import the resources and begin managing them as code. Use this workflow to import large batches of resources. For instructions on importing single resources or small batches of resources on your local workstation, refer to [Import an existing resource](/terraform/language/import/single-resource). 
