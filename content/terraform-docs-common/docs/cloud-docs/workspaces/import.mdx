...
You can also connect your local workstation to HCP Terraform and use the CLI to copy the configuration to your HCP Terraform workspace. Refer to [Connect to HCP Terraform](/terraform/cli/cloud/settings) for instructions.
...
In the **IaC** column, HCP Terraform indicates one of the following statuses for resources returned by the query: 
  ...
  - **Unknown** indicates that the resource has the same [identity](/terraform/language/import#resource-identity) as a resource applied by an older verison of Terraform or older provider version. 
