...
When publishing modules to the private registry, the registry downloads the module source code. It uses API tokens to control access so consumers do not need access to the module's source repository, even when running Terraform from the command line. Additionally, the private registry uses your [version control system (VCS) integrations][vcs] and delegates management tasks, such as handling new version releases, to your VCS provider. The only manual tasks are adding a new module and deleting module versions.
...
- You must configure a VC provider connection. Refer to [Connect to VCS Providers](/terraform/cloud-docs/vc) for more information. 
- The source directory settings in your VCS must specify the path to the module configuration files.

### Repository

The registry automatically detects module names and versions. Your module repository meets all of the following requirements before adding it to the registry.
...
Specify a prefix to filter repository Git tags when using the [tag-based publishing type](#tag-based-publishing-considerations) in a repository that contains code for multiple modules. Without a prefix, HCP Terraform publishes new versions of modules tagged with a valid semantic version.
...
Modules in the repository must use the following three-part naming convention:
...
The interface for publishing modules and the registry behavior depends how the **Publish modules to your private registry from a single repository** option. Refer to [Enable publishing from a single repository](#enable-publishing-from-a-single-repository) for more information about this setting.
...
1. In the **Module Name** field, specify a name for the module as it will appear in the registry. HCP Terraform and Terraform Enterprise use the name in the module address. 
