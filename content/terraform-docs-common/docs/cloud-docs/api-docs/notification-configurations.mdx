...
description: >-
 Use the HCP Terraform API's workspace notification configuration endpoints to read, create, update, verify, and delete workspace notification configurations.
---
...
HCP Terraform can send workspace notifications for run state transitions and workspace events. You can specify a destination URL, request type, and what events will trigger the notification. Each workspace can have up to 20 notification configurations, and they apply to all runs for that workspace.
