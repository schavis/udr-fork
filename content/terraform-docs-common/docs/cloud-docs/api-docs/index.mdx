...
<!-- BEGIN: TFC -->
HCP Terraform<!-- END: TFC --><!-- BEGIN: TFEnterprise:only name:api-overview -->Terraform Enterprise<!-- END: TFEnterprise:only --> provides an API for a subset of its features. If you need assistance or want to submit a feature request, visit the [HashiCorp support center](https://support.hashicorp.com/hc/en-us) and open a ticket.
...
<!-- BEGIN: TFC:only -->HCP Terraform relies on a HashiCorp-developed blob storage service for storing statefiles and multiple other pieces of customer data, all of which are documented on our [data security page](/terraform/cloud-docs/architectural-details/data-security). 
<!-- END: TFC:only -->
<!-- BEGIN: TFEnterprise:only name:blob-storage -->Terraform Enterprise relies on a HashiCorp-developed blob storage service for storing state files and other pieces of customer data.
<!-- END: TFC:only --> This service does not require you to submit a bearer token with each request. Instead, each URL includes a securely-generated secret and is only valid for 25 hours.
