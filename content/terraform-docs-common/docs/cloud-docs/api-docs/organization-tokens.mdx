...
| `data.attributes.expired-at`  | string | `null`  | The UTC date and time that the organization token will expire, in ISO 8601 format. If omitted or set to `null`, the token will expire in 2 years from creation. |
