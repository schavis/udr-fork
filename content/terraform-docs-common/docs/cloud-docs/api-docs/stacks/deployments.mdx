...
# Stack deployments API

Stack deployments are created in a Stacks `deployment.tfdeploy.hcl` file. A deployment consists of a deployment group, at least one deployment run, and at least one deployment step.
...
| `GET`    | `/stacks/:stack_id/stack-deployments/:deployment_name/stack-deployment-runs`                         | Call this endpoint to [list deployment runs for a deployment name](#list-stack-deployments).                                   |
