...
- `manual` (default) - Creates an empty Stack configuration. You will have to retrieve the `upload-url` after creation (see the upload-url endpoint documentation below) and upload a configuration archive to enable the Stack to prepare and plan the configuration. If the Stack is connected to a VCS repo, this mode can only be used if the `speculative` attribute is also set to `true`.
- `fetch` - Fetches new Stack configuration content from the connected VCS repository. This mode is not valid for stacks that are not connected to a VCS repo.
