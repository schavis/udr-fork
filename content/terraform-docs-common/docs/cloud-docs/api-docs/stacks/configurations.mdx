...
# Stack configurations API

Stack configurations represent the infrastructure components that constitute a Stack.
...
This endpoint creates an empty Stack configuration for the specified Stack. A configuration file will have to be uploaded to the Stack configurations upload url in order for the Stack prepare job to run and generate plans. This endpoint cannot be used if the Stack is connected to a VCS repo.
...
This endpoint creates a Stack configuration for a VCS backed Stack by pulling it from the head of the Stacks branch.
...
This endpoint fetches the Stack configuration upload url. This url is used to upload a raw configuration file for a manually created Stack configuration.
