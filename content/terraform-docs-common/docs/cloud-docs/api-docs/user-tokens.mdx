...
| `data.attributes.expired-at`  | string | `null`  | The UTC date and time that the User Token will expire in ISO 8601 format. If omitted or set to `null`, the token will expire two years from the date of creation. |
