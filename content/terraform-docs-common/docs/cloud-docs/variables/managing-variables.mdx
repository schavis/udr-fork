...
In Stacks, you can use the [`store` block](/terraform/language/block/stack/deploy/store) to access variable sets. Your Stack must be able to access the variable set you are targeting, by giving your Stack's project access, or by making the variable set globally available.
