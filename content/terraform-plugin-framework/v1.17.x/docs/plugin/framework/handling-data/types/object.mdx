...
Objects values that have additional metadata are supported in most framework [schemas](/terraform/plugin/framework/handling-data/schemas), [nested attribute types](/terraform/plugin/framework/handling-data/attributes#nested-attribute-types), and [block types](/terraform/plugin/framework/handling-data/blocks). For example (non-exhaustive):
...
Object values that only have type information are supported in most framework [schemas](/terraform/plugin/framework/handling-data/schemas) and [nested attribute types](/terraform/plugin/framework/handling-data/attributes#nested-attribute-types).
Each concept (provider, resource, data source, etc.) has a separate `ObjectAttribute`, for example (non-exhaustive):
