---
page_title: Set nested blocks
description: >-
  Learn to implement the set nested block type with the Terraform plugin framework.
# START AUTO GENERATED METADATA, DO NOT EDIT
created_at: 2026-03-05T16:11:31.000Z
last_modified: 2026-03-05T16:11:31.000Z
...
Set nested blocks are supported in most framework [schemas](/terraform/plugin/framework/handling-data/schemas) and [block types](/terraform/plugin/framework/handling-data/blocks).
Each concept (provider, resource, data source, etc.) has a separate `SetNestedBlock` which can be used to add a set nested value to a schema. For example (non-exhaustive):
