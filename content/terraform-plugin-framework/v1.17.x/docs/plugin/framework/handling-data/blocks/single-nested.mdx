...
Single nested blocks are supported in most framework [schemas](/terraform/plugin/framework/handling-data/schemas) and [block types](/terraform/plugin/framework/handling-data/blocks).
Each concept (provider, resource, data source, etc.) has a separate `SingleNestedBlock` which can be used to add a single nested value to a schema. For example (non-exhaustive):
