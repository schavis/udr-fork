...
created_at: 2026-03-05T16:12:55.000Z
last_modified: 2026-03-05T16:12:55.000Z
...
This page describes single attribute, parameter, and type validation concepts that can for any framework schema, such as [data sources](/terraform/plugin/framework/data-sources/validate-configuration), [resources](/terraform/plugin/framework/resources/validate-configuration), [provider schema](/terraform/plugin/framework/providers/validate-configuration), etc.
...
During execution of the [`terraform validate`](/terraform/cli/commands/validate), [`terraform plan`](/terraform/cli/commands/plan), [`terraform apply`](/terraform/cli/commands/apply) and [`terraform destroy`](/terraform/cli/commands/destroy) commands, Terraform calls the provider's validate RPCs, which are all named `Validate<Type>Config`, for example: `ValidateProviderConfig`, `ValidateResourceConfig`, `ValidateDataSourceConfig`, `ValidateActionConfig`, etc.
