---
page_title: Map attributes
description: >-
  Learn how to use map attributes with the Terraform plugin framework.
...
Map attributes are supported in most framework [schemas](/terraform/plugin/framework/handling-data/schemas) and [nested attribute types](/terraform/plugin/framework/handling-data/attributes#nested-attribute-types).
Each concept (provider, resource, data source, etc.) has a separate `MapAttribute` which can be used to add a map value to a schema. For example (non-exhaustive):
