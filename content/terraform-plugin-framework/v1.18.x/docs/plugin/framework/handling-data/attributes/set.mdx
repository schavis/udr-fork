---
page_title: Set attributes
description: >-
  Learn how to use set attributes with the Terraform plugin framework.
...
Set attributes are supported in most framework [schemas](/terraform/plugin/framework/handling-data/schemas) and [nested attribute types](/terraform/plugin/framework/handling-data/attributes#nested-attribute-types).
Each concept (provider, resource, data source, etc.) has a separate `SetAttribute` which can be used to add a set value to a schema. For example (non-exhaustive):
