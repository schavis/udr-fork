...
List attributes are supported in all framework [schemas](/terraform/plugin/framework/handling-data/schemas) and [nested attribute types](/terraform/plugin/framework/handling-data/attributes#nested-attribute-types).
Each concept (provider, resource, data source, etc.) has a separate `ListAttribute` which can be used to add a list value to a schema. For example (non-exhaustive):
