---
page_title: Dynamic types
description: >-
  Learn how to implement dynamic types with the Terraform plugin framework.
...
Dynamic values can be added to most framework [schemas](/terraform/plugin/framework/handling-data/schemas) and [nested attribute types](/terraform/plugin/framework/handling-data/attributes#nested-attribute-types).
Each concept (provider, resource, data source, etc.) has a separate `DynamicAttribute`, for example (non-exhaustive):
