...
Managing thousands of cloud resources across regions, environments, and teams is complex. Tags are key-value pairs that help you manage, identify, organize, locate, and filter resources. A clear tagging strategy becomes essential as your resource count grows.
...
**Automate resource management:** Tags allow automation to act on resources without manual intervention. For example, a lifecycle policy can shut down all instances tagged `Environment: dev` outside business hours. A cleanup job can delete any resource tagged `Temporary: true` after 30 days. Without consistent tags, these automations cannot reliably identify their targets.
...
These tags attach to every AWS EC2 instance that Terraform creates from this resource block. Tags like `Environment` and `CostCenter` let you filter resources in the AWS console, build cost allocation reports, and scope automation policies to specific groups of instances.

The AWS and GCP Terraform providers let you add default tags to all resources they create. Default tags make it easier to implement a consistent tagging strategy across the resources you manage with Terraform. Default tags ensure that all resources have the minimum required tags but you can override these default tags on a per-resource basis. The Azure provider does not offer equivalent default tags; use [Azure Policy](https://learn.microsoft.com/en-us/azure/governance/policy/concepts/effects#modify) to enforce tag inheritance for Azure resources.
...
In this section of [Lifecycle management](/well-architected-framework/optimize-systems/lifecycle-management), you learned how to tag resources using infrastructure as code and enforce tagging policies.
