...
This API supports only monthly granularity until support for daily-level is added. Until then, 
the `start_time` and `end_time` values in the reponse are normalized to align with full calendar months. 
This normalization occurs whether the times are explicitly provided in the request or defaulted by Vault.

- The `start_time` is normalized to the start of the month in which the effective `start_time` falls.
- The `end_time` is normalized to the end of the month in which the effective `end_time` falls.
...
months in the requested time range. Vault normalizes the effective `start_time`
and `end_time` to full clandar months - adjusting the `start_time` to the 
beginning of the month and `end_time` to the end of the month, even if those 
times were defaulted.
