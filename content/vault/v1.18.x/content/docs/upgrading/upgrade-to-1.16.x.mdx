...
To guard against potential Denial-of-Service (DoS) attacks, this release
introduces several Vault listener options that enforce payload size limits applied to incoming JSON request bodies.
These settings are configured on a listener and give administrators granular control over the
structure and size of JSON payloads that Vault will process and the configuration and performance characteristics of their Vault cluster.
We've chosen defaults that are intentionally generous to accommodate a wide range of
legitimate use cases, and which should guard against most malicious or malformed requests.
...
with more than 10,000 keys is extremely rare, so the default of 10,000 provides a high ceiling for normal operations
