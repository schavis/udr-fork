...
- `schema` - (string, optional) - Defines the LDAP schema used to store entry passwords. Supported values are `openldap` and `ad`. This parameter is optional for `openldap`, but required when using the `ad` schema.
