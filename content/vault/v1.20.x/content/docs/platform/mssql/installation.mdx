...
1. You can now create an asymmetric key using the transit key set up earlier:
    ...
    The above steps use the latest version of the specified Transit key as the asymmetric Key Encryption Key (KEK).  
    To use a different version of the Transit key, please specify the version as shown below.  Specifying the version 
