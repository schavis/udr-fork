...
<Note>
  When all enabled audit devices become unavailable, Vault in effect becomes unavailable as well. To prevent audit-related unavailability of Vault, HashiCorp recommends that at least two audit devices be enabled.
</Note>
...
<Warning title="Blocking device failures">
  Audit devices can experience blocking failures, where Vault cannot determine whether the write to the audit device has succeeded and hangs while waiting. Blocking failures are unlikely to happen with local disk devices but may happen with network-based devices. When any configured audit device experiences a blocking failure, Vault will not complete any requests until the blocking audit device becomes healthy again.
</Warning>
