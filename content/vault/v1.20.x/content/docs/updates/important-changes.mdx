...
Unfortunately a private repo has been listed in the go.mod file. This will
cause the `go mod tidy` command to fail.

#### Recommendation

Either remove the line referencing `hashicorp/go-cmp` or build from main.
