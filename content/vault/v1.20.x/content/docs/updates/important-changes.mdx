...
### File audit devices require explicit configuration to execute files and write to plugin directory ((#audit-file-security))

| Change       | Affected version                          | Fixed version
| ------------ | ----------------------------------------- | --------------------
| Breaking     | 1.20.1                                    | N/A

You must set `allow_audit_log_prefixing` to `true` in your server configuration
to enable `file` audit devices with the `prefix` option. Additionally, `file`
audit devices cannot write to the plugin directory, including subdirectories,
and cannot use file modes with executable permissions (e.g., 0777, 0755).

#### Recommendation

If you use the `prefix` option with `file` audit devices, you have three options:

1. Add `allow_audit_log_prefixing = true` to your Vault server configuration.
1. Configure the `file` audit device to write log files outside the plugin directory.
1. Use non-executable file modes (e.g., 0644, 0666).
