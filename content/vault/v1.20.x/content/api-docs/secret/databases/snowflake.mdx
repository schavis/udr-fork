...
- `max_connection_lifetime` `(string: "0s")` - Specifies the maximum amount of
  time a connection may be reused. If <= `0s` connections are reused forever. Be aware
  that Snowflake's default session policy limits connections to 4 hours before being renewed,
  but can be [changed](https://docs.snowflake.com/en/user-guide/session-policies#session-policies).
