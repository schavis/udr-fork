...
description: >-
    The `/sys/billing/overview` endpoint returns a monthly overview of billing data.
---
...
The `/sys/billing/overview` endpoint returns a monthly overview of consumption data relevant to consumption-based licensing for the current and previous months. Vault updates current-month metrics every 10 minutes. Refer to the `updated_at` field in monthly response data to determine when Vault last updated the billing metrics for each month.
...
- `refresh_data` `(boolean: false)` - When set to `true, Vault recalculates and
  all billing metrics and stores the new metrics. Leave `refresh_data` unset or
  explicitly set to `false` to return data as of when it was last calculated.
  Be aware that recalculating metrics consumes server resources and may cause
  performance issues if done too frequently.
