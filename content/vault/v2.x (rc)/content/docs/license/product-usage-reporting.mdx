...
description: >-
  Learn about the anonymized data HashiCorp collects automatically for product usage reporting and how to enable or disable data collection.


# START AUTO GENERATED METADATA, DO NOT EDIT
created_at: 2026-01-27T17:15:25-08:00
last_modified: 2026-03-26T19:06:45.000Z
...
To help guide product development efforts, HashiCorp collects usage data about how Vault clusters and features are used. Except for the metadata listed in [usage metadata](#usage-metadata-list), product usage metrics are numeric only. See [usage metrics list](#usage-metrics-list) below for a description of each of product usage metric.

Product usage data is collected as part of the [license utilization reporting](/vault/docs/license/utilization) process and managed in a compliant manner.
...
If you opt out of product usage reporting, Vault's [license utilization reporting payloads](/vault/docs/license/utilization/metrics#sample-payload) will continue to include the names and **default values** of product usage metrics. For example, even if your cluster has 100 namespaces, the `vault.namespaces.count` metric will still report 0.
...
```
OPTOUT_PRODUCT_USAGE_REPORTING=true
```

Vault will log the following messages in its server logs:
...
HashiCorp collects the following product usage metrics. All of these metrics are numerical, and contain no sensitive values or additional metadata.
...
HashiCorp collects the following product data in the [`metadata`](/vault/docs/license/utilization/metrics/#metadata) section of each snapshot:
