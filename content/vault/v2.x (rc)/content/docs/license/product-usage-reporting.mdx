...
To help guide product development efforts, HashiCorp collects usage data about how Vault clusters and features are used. Except for the metadata listed in [Usage metadata list](#usage-metadata-list), product usage metrics are numeric only. Refer to [Usage metrics list](#usage-metrics-list) for a description of each product usage metric.
...
Be sure to apply the same configuration or environment variable to all nodes on a Vault cluster. When leadership changes, the new leader node's settings take effect.

If your HCL configuration file and environment variable differ, the environment variable setting takes precedence.

If you opt out of product usage reporting, Vault's [license utilization reporting payloads](/vault/docs/license/utilization/metrics#sample-payload) continue to include the names and **default values** of product usage metrics. For example, even if your cluster has 100 namespaces, the `vault.namespaces.count` metric reports 0.

Opting out of product usage reporting does not affect snapshots taken before the opt-out settings took effect.

### HCL configuration

Add the following to your Vault HCL configuration file. If Vault is already running, trigger a configuration reload by sending a `SIGHUP` signal to the Vault process.
...
Set the following environment variable for the Vault process. If Vault is already running, restart the process before the environment variable takes effect.
