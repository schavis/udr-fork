...
last_modified: 2026-04-03T20:34:34.000Z
...
- Secret engine [billing metrics] (../concepts/consumption-billing/billing_metrics)
...
- `snapshots_records` - An array of snapshot records. Each snapshot record is a structure that
  represents a single data collection
  - `snapshot_version` - The version of the snapshot package that produced the reporting
    snapshot.
  - `data` - The binary-encoded contents of the snapshot
  ...
  - `license` - A structure containing information about the license in use when the snapshot was taken
    - `id` - The license ID
    ...
    - `decoded_snapshot` - The decoded contents of the snapshot, which is a structure containing the following fields:
      ...
      - `gauges` - A map of representations of snapshot metrics contained within
        this snapshot
      ...
      - `metadata` - Optional product-specific metadata
        ...
        - `add_ons` - For IBM PAO licenses, the add-ons associated with the entitlement. For HashiCorp licenses, this field will always be empty.
        - `edition` - For IBM PAO licenses, the edition associated with the entitlement. For HashiCorp licenses, this field will always be empty.
        - `issuer` - The issuer of the license
        - `license_start_time` - The start time of the license
        - `license_expiration_time` - The expiration time of the license
        - `license_termination_time` - The termination time of the license, if applicable
