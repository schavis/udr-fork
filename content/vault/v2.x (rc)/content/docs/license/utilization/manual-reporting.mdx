...
There are two ways to opt out of Vault's automated license utilization reporting:
...
Add the following to your Vault HCL configuration file. When Vault is already running, trigger a configuration reload by sending a `SIGHUP` signal to the Vault process.
...
Set the following environment variable for the Vault process. When Vault is already running, restart the process before the environment variable takes effect.
...
You can send the license utilization reporting payload to HashiCorp using a web browser or directly to the API:
...
When the upload is successful, HashiCorp's reporting API returns an HTTP 202 Accepted response status code.
...
When the upload fails, the reporting API returns an error message and 4XX or 5XX status code. For example, using a payload with a tampered signature:
