...
1. Configure a sync destination using the below approaches
  ...
  - Using **Workload identity federation (WIF)**:
     ...
     between Vault and GCP. Use WIF to eliminate the need to manage long-lived
...
Workload identity federation (WIF) lets the GCP Secret Manager sync destination
authenticate to GCP by having Vault exchange short-lived identity tokens for
GCP access tokens instead of managing long-lived service account credentials.

To configure WIF for GCP Secret Manager sync:
...
1. Configure a
   [workload identity pool and provider](https://cloud.google.com/iam/docs/workload-identity-federation-with-other-providers)
   in GCP to establish a trust relationship with Vault.
   ...
   - The audience should be configured based on your workload identity pool. The default format is typically
     `//iam.googleapis.com/projects/<PROJECT_NUMBER>/locations/global/workloadIdentityPools/<POOL_ID>/providers/<PROVIDER_ID>`.

1. Configure the GCP Secret Manager sync destination with the identity token parameters,
...
[API documentation](/vault/api-docs/system/secrets-sync#create-update-gcp-secret-manager-destination).
