...
Vault replicates audit device configurations to all nodes within a cluster. Vault Enterprise also replicates audit device configurations to both performance and disaster recovery secondary clusters. If you configure the audit device as [`local`](/vault/api-docs/system/audit#local) to the cluster, Vault does not replicate the audit device to performance secondary clusters, but it still replicates the configuration to disaster secondary clusters. Before you enable an audit device, ensure that all Vault nodes to which the audit device configuration replicates can successfully log to that audit device.
