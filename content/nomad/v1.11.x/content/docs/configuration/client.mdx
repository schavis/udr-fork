...
- `network_interface` `(string: varied)` - Specifies the name of the interface
  ...
  format][go-sockaddr/template]. The fingerprinted interface will be added to a
...
- `host_network` <code>([host_network](#host_network-block): nil)</code> - Registers
  ...
  Although this parameter defaults to nil, a host network named "default" is created
...
the node that can be used when port mapping. Multiple host network blocks
can be defined with the same name in order to assign multiple interfaces
or cidr ranges to the same host network. When used in a job, Nomad will
reserve a port on the first available address for the host network.
...
```hcl
client {
  // Interface en1 will be added to the existing "default" host_network
