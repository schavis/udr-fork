...
Saving snapshots no longer requires a management token. We added find-grained
...
- If ACLs are enabled, the [`nomad operator snapshot agent`
  command](/nomad/commands/operator/snapshot/agent) requires a token with with
