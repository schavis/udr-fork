...
last_modified: 2026-02-25T03:41:54.000Z
...
1. Create a new `systemd` service for Terraform Enterprise by creating a `/etc/systemd/system/terraform-enterprise.service` file with [the contents on the Docker Installation guide](/terraform/enterprise/deploy/docker). First, verify your Docker installation paths:
   ...
   Use the `sudo tee` command to create the systemd service file. In the following example, Docker is installed at `/usr/bin/docker`. Adjust the paths in the configuration if your Docker installation differs:
...
1. Stop Terraform Enterprise (Replicated). Ensure the application has fully stopped before proceeding.
   ...
   ```
1. Back up your data. If you have already backed up your data, proceed forward. If not, or if you want another backup for safekeeping, do the following.
...
Redis is a _required service_ for running Terraform Enterprise in Kubernetes. If your deployment is operating in [`external` mode](/terraform/enterprise/deploy/reference/configuration#external), you must deploy an external Redis server.
