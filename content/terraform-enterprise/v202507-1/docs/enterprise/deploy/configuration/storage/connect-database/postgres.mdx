...
<Note>
  Workload Identities are not supported.
</Note>
...
- Set the `TFE_DATABASE_USER` as the managed identity name.

You need to ensure that the managed identity user has the [required permissions](#user) on the database.
If you are migrating from password-based authentication to MSI-based authentication, the database and schemas are initially owned by the previous user. 
You need to ensure that the managed identity user has the required permissions or is the owner of the database and schemas inside it.
