...
2. Site admins can now change their password even when logged in using SSO. An alert also displays in the SAML settings page. The alert staties that admins can log in with their username and password even when SSO is enabled.
