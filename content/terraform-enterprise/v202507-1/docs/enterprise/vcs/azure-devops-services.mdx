...
3.  Provide HCP Terraform with the application ID, MS Entra Tenant ID and key. Then, request VCS access.
...
3.  Once the app is registered, click **API permissions** under the **Manage** section from the application overview, click **Azure DevOps**, then add **vso.code** and **vso.code_status** permissions.
...
4.  Click **Certificates and secrets**, choose the **Client secrets** tab, then click **New client secret**. Follow the instructions shown to create a client secret.

5.  Click **Overview** and leave this page open in a browser tab. In the next step, you will copy and paste the unique **Application (client) ID**, **Directory (tenant) ID** and **Client Secret** from this page.
