...
You can use `filter[n]` to combine combinations of statuses and enforcement levels. Policy set outcomes with an errored status do not have an enforcement level. The filter returns all outcomes when at least one outcome matches the filter criteria.
...
The API applies filters when retrieving outcomes, but it does not filter within the individual outcomes. Instead, the filter returns all policy set outcomes that include at least one outcome that matches the filter criteria.
If the response has a failed outcome, then all the outcomes for that policy set are returned.
