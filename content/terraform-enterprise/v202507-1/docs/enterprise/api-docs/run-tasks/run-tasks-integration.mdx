...
The PATCH request submits a JSON object with the following properties as a request payload. This payload is also described in the [JSON API schema for run task results](https://github.com/hashicorp/web-unified-docs/content/terraform-docs-common/public/schema/run-tasks/runtask-result.json).
