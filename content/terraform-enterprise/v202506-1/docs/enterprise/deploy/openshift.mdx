...
env:
  variables:
    ...
    TFE_REDIS_SIDEKIQ_CERT_PATH: <Path to the client certificate file to be used for mTLS authentication with Redis server>
    TFE_REDIS_SIDEKIQ_PATH: <Path to the Certificate Authority file user to validate the certificate>
    TFE_REDIS_SIDEKIQ_KEY_PATH: <Path to the private key file corresponding to the client certificate>
