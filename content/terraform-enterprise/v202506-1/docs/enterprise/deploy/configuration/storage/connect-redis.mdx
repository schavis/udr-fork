...
- Specify the path to the CA certificate file in the `TFE_REDIS_CA_CERT_FILE` variable.
- Specify the path to the client certificate file in the `TFE_REDIS_CLIENT_CERT_FILE` variable.
- Specify the path to the client key in the `TFE_REDIS_CLIENT_KEY_FILE` variable.
...
- Set the `TFE_REDIS_SIDEKIQ_CERT_PATH` variable to set the path to the client certificate file to be used for mTLS authentication with Redis server.
- Set the `TFE_REDIS_SIDEKIQ_PATH` variable to set the path to the Certificate Authority file user to validate the certificate.
- Set the `TFE_REDIS_SIDEKIQ_KEY_PATH` variable to set the path to the private key file corresponding to the client certificate.
