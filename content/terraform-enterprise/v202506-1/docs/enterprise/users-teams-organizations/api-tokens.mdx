...
The [legacy API](/terraform/enterprise/api-docs/team-tokens#legacy-team-tokens-api-reference) only operate on the legacy token. Generating a new legacy token invalidates the previous legacy token.

The [non-legacy API](/terraform/enterprise/api-docs/team-tokens#team-tokens-api-reference) supports multiple valid team tokens. When a new token is generated, existing tokens remain valid.
