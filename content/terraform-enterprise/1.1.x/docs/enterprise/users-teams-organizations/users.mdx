...
User accounts belong to individual people. Each user can be part of one or more [teams](/terraform/enterprise/users-teams-organizations/teams), which are granted permissions on projects and workspaces within an organization. A user can be a member of multiple [organizations][].
...
Only an HCP Terraform or Terraform Enterprise user can own a GitHub App token. Team and organization API tokens are not able to own a GitHub App token.
