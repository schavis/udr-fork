...
Sets a fixed number of web worker processes for the Atlas Rails application.

By default, Terraform Enterprise automatically scales web workers based on CPU cores (CPU count ÷ 2, minimum 1, maximum 10). Use this setting when you need predictable resource allocation or have determined an optimal worker count for your workload.

Higher values can improve response times under heavy load but increase memory usage. 

Defaults to automatic scaling (CPU count ÷ 2, minimum 1, maximum 10).
...
When enabled, Puma worker processes restart based on memory thresholds or scheduled intervals. This prevents memory leaks and ensures consistent performance.
...
Use standard cron syntax. For example, `0 2 * * *` restarts workers daily at 2 AM. 

### `TFE_ATLAS_ROTATE_MEMORY_THRESHOLD`

Memory threshold that triggers automatic Puma process rotation. Specify using human-readable format (e.g., `4GB`, `2048MB`).
...
Terraform Enterprise runs two Sidekiq worker pools: one for all job queues and another exclusively for critical jobs. This ensures important operations aren't delayed by lower-priority work.
