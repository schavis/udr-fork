...
For more information please see the [Azure documentation](https://learn.microsoft.com/en-us/azure/azure-cache-for-redis/cache-managed-identity).
...
- Set the `TFE_REDIS_SIDEKIQ_PASSWORDLESS_AZURE_USE_MSI` variable to `true` if your Sidekiq Redis instance requires to connect using Azure Entra as well.
...
- Set the `TFE_REDIS_USER` to the managed identity. Redis cache expects this while issuing the `AUTH` command.
- Set the `TFE_REDIS_SIDEKIQ_USER` to the managed identity. Redis cache expects this while issuing the `AUTH` command. Defaults to the same value as `TFE_REDIS_USER` if not set.
