...
By default, a project uses the organization's [default execution mode](/terraform/enterprise/users-teams-organizations/organizations#organization-settings) to choose the execution platform for a project. Alternatively, you can instead choose a custom execution mode for a project.
...
To disable remote execution for a project, change its execution mode to "Local". This mode lets you perform Terraform runs locally with the [CLI-driven run workflow](/terraform/enterprise/run/cli).
